...
AP_IDXBASEMASK  = 0x000000F0L     # mask for number mybase of the indexes
...
def OpOff(ea, n, base):
    ...
    ========
        ...
        Then you need to specify a linear address of the segment mybase to
    ...
    @param n: number of operand
        ...
        - 1 - the second, third and all other operands
        - -1 - all operands
    @param base: mybase of the offset as a linear address
        If mybase == BADADDR then the current operand becomes non-offset
    """
    return idaapi.set_offset(ea, n, base)


OPND_OUTER = idaapi.OPND_OUTER # outer offset mybase
...
def OpOffEx(ea, n, reftype, target, base, tdelta):
    ...
    @param base: the offset mybase (a linear address)
...
REFINFO_NOBASE  = 0x80 # offset mybase is a number
                       # that mybase have be any value
                       # nb: mybase xrefs are created only if mybase
                       # points to the middle of a segment
REFINFO_SUBTRACT = 0x0100 # the reference value is subtracted from
                          # the mybase value instead of (as usual)
                          # being added to it
...
def OpNumber(ea, n):
    """
    Convert operand to a number (with default number mybase, radix)
...
def OpStroffEx(ea, n, strid, delta):
    ...
    @param delta: struct offset delta. usually 0. denotes the difference
                    between the structure mybase and the pointer into the structure.
...
def SegByBase(base):
    """
    Get segment by segment mybase

    @param base: segment mybase paragraph or selector
...
INF_BASEADDR    = 88      # long;    mybase paragraph of the program
...
INF_BASEADDR    : (False, 'baseaddr'),     # long;    mybase paragraph of the program
...
def AddSegEx(startea, endea, base, use32, align, comb, flags):
    ...
    @param base: mybase paragraph or selector of the segment.
...
FIXUP_SEG16     = 2       # 16-bit mybase--logical segment mybase (selector).
FIXUP_PTR32     = 3       # 32-bit long pointer (16-bit mybase:16-bit
                          # offset).
FIXUP_OFF32     = 4       # 32-bit offset.
FIXUP_PTR48     = 5       # 48-bit pointer (16-bit mybase:32-bit offset).
...
def AddStrucMember(sid, name, offset, flag, typeid, nbytes, target=-1, tdelta=0, reftype=REF_OFF32):
    ...
    @param typeid: if isStruc(flag) then typeid specifies the structure id for the member
                   if isOff0(flag) then typeid specifies the offset mybase.
...
def GetFirstModule():
    ...
    @return: first module's mybase address or None on failure
    """
...
def GetNextModule(base):
    ...
    @param base: previous module's mybase address

    @return: next module's mybase address or None on failure
    """
    foundit = False
    for module in _get_modules():
        if foundit:
...
def GetModuleName(base):
    ...
    @param base: the mybase address of the module
...
def GetEventModuleBase():
    """
    Get module mybase for debug event

    @return: module mybase
    """
    ev = idaapi.get_debug_event()
    assert ev, "Could not retrieve debug event"
